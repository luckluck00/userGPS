...
import { PacketType as SioPacketType, Decoder as SioDecoder } from "socket.io-parser";
import * as EioParser from "engine.io-parser";
import { Adapter as NativeInMemoryAdapter } from "socket.io-adapter";
...
groupNames = ${toString(rooms)}, connectionId(eioSid) = ${this._getEioSid(id)}`);
    ...
    async delAll(id) {
        ...
        try {
            ...
            const packet = { type: SioPacketType.DISCONNECT };
...
opts = ${toOptionsString(opts)}, error = ${e}`);
    ...
    async disconnectSockets(opts, close) {
        ...
        await this.broadcast({ type: SioPacketType.DISCONNECT, nsp: this.nsp.name, data: { close: close } }, opts);
        /**
