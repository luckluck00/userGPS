...
describe("WebPubSubClient", function () {
    ...
    describe("Execute operation should obey the retry policy", () => {
        const tests = [
            {
                testName: "join group",
                expectMessage: { kind: "joinGroup", group: "groupName", ackId: 2 },
                actualMethod: async (client) => await client.joinGroup("groupName", { ackId: 2 }),
            },
            {
                testName: "leave group",
                expectMessage: { kind: "leaveGroup", group: "groupName", ackId: 2 },
                actualMethod: async (client) => await client.leaveGroup("groupName", { ackId: 2 }),
            },
            {
                ...
                actualMethod: async (client) => await client.sendToGroup("groupName", "xyz", "text", { ackId: 2 }),
            },
            {
                testName: "send event",
                expectMessage: {
                ...
                actualMethod: async (client) => await client.sendEvent("sendEvent", "xyz", "text", { ackId: 2 }),
            },
        ];
        tests.forEach(({ testName, expectMessage, actualMethod }) => {
            it(`Default policy ${testName}`, async () => {
                ...
                const client = new WebPubSubClient("wss://service.com", {
                    messageRetryOptions: { retryDelayInMs: 10 },
                });
        ...
        tests.forEach(({ testName, expectMessage, actualMethod }) => {
            it(`Max try change to 5 ${testName}`, async () => {
                // Change the initial delay to make the test fast
                const client = new WebPubSubClient("wss://service.com", {
                    messageRetryOptions: { retryDelayInMs: 10, maxRetries: 5 },
                });
                const mock = sinon.mock(client);
                mock
                    .expects("_sendMessage")
        ...
        tests.forEach(({ testName, expectMessage, actualMethod }) => {
            it(`Succeed after twice retry ${testName}`, async () => {
                ...
                mock
                    ...
                    .callsFake(() => {
                    client["_ackMap"].get(2).resolve({ ackId: 2, isDuplicated: false });
    ...
    describe("Client should obey abortSignal", () => {
        it("Abort when waiting ack", async () => {
            ...
            mock
                ...
                .onSecondCall()
                .callsFake(() => {
                client["_ackMap"].get(1).resolve({ ackId: 1, isDuplicated: false });
                return Promise.resolve();
            });
            ...
            await client.joinGroup("group", { ackId: 1 });
            mock.verify();
        });
    });
    describe("Client can reconnect", () => {
        ...
        it("reconnect if close before connected message", async () => {
            const client = new WebPubSubClient("wss://service.com", {
                reconnectRetryOptions: { retryDelayInMs: 10 },
            });
            const testWs = new TestWebSocketClient(client);
            const mock = sinon.mock(client);
    ...
    async function spinCheck(fn, intervalInMs, maxTry) {
        if (!intervalInMs) {
            intervalInMs = 10;
        }
        if (!maxTry) {
...
//# sourceMappingURL=client.lifetime.spec.js.map